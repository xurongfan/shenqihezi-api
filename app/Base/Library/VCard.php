...
class VCard
{
    ...
    private function addMedia($property, $url, $include = true, $element)
    {
        if ($include) {
            ...
            if (!$value) {
                throw new \Exception('Nothing returned from URL.');
            }
            ...
            if (preg_match('/^image\//', $mimetype) !== 1) {
                throw new \Exception('Returned data aren\'t an image.');
            }
    ...
    public function addIMPP($impp, $type='')
    {
        $this->setProperty(
            'impp',
            'IMPP' . (($type != '') ? ';' . $type : ''),
...
//            header($header);
//        }
        if ($this->getCharset() == 'gbk') {
            $output = $this->utf2gbk($output);
        }
        // echo the output and it will be a download
        $headers = [
            'Content-type' => $this->getContentType() . '; charset=' . $this->getCharset(),
            'Content-Disposition' => 'attachment; filename="' . $this->getFilename() . '.' . $this->getFileExtension().'";filename*=utf-8\'\''. $this->getFilename() . '.' . $this->getFileExtension(),
    ...
    public function getHeaders($asAssociative)
    {
        ...
        $contentDisposition = 'attachment; filename="' . $this->getFilename() . '.' . $this->getFileExtension().'";filename*=utf-8\'\''. $this->getFilename() . '.' . $this->getFileExtension();
    ...
    public function setFilename($value, $overwrite = true, $separator = '_')
    {
        ...
        $value = strtolower($value);//$this->decode(
    ...
    public function addIsCompany($is)
    {
        $this->setProperty(
            'is_company',
            'IS_COMPANY',
    ...
    public function utf2gbk($content)
    {
